...
function resolveGeminiPassthroughTimeoutMs() {
...
function isGeminiPassthroughCandidate(provider: Provider) {
  return provider.protocol === "google";
}

function buildUpstreamUrl(provider: Provider, model: string, requestUrl: string) {
...
async function recordRequestLogSafe(input: Parameters<typeof createRequestLog>[0]) {
  try {
    ...
    console.error("[gemini] request log failed", error);
  }
}
...
export const geminiPassthroughRoutes = new Hono();

geminiPassthroughRoutes.post("/google/v1beta/models/*", async (c) => {
  const pathMatch = c.req.path.match(
    /^\/google\/v1beta\/models\/([^/]+):generateContent$/
  );
  const modelSlug = pathMatch ? decodeURIComponent(pathMatch[1]) : "";
  if (!modelSlug) {
  ...
  const usageEstimate = estimateUsage("gemini", parsedBody as Record<string, unknown>);
  ...
  const timeoutMs = resolveGeminiPassthroughTimeoutMs();
  ...
  for (const provider of candidates) {
    ...
    const upstreamUrl = buildUpstreamUrl(provider, modelSlug, c.req.url);
    ...
    try {
        ...
        return new Response(
          wrapStreamWithFinalizer(clientBody, {
            ...
            onError: async (streamError) => {
              ...
              console.warn("[gemini] upstream stream failed after first byte", {
                providerId: provider.id,
                providerName: provider.name,
                errorType,
                message,
      ...
      recordFailure(provider.id, errorType);
      console.warn("[gemini] upstream response failed before first byte", {
        providerId: provider.id,
        providerName: provider.name,
        status: upstreamResponse.status,
        errorType,
      ...
      recordFailure(provider.id, errorType);
      console.warn("[gemini] upstream request failed before first byte", {
        providerId: provider.id,
        providerName: provider.name,
        errorType,
        message,
