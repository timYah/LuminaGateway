...
- [x] **T-49.4** 请求级日志：记录 provider、模型、错误类型、耗时、结果（成功/失败）
- [x] **T-49.5** ✅ **验证**：编写/更新测试覆盖健康检查与失败分类统计，运行 `npm run test -- gatewayService && npm run test -- upstreamService`
- [x] **T-49.6** ✅ **验证**：运行 `npm run lint && npm run typecheck && npm run test`
- [x] **T-49.7** ✅ **验证（Admin）**：运行 `npm run dev` 并使用 agent-browser skill 验证健康状态与失败分类展示
...
- [x] **T-50.6** ✅ **验证（Admin）**：运行 `npm run dev` 并使用 agent-browser skill 验证仪表盘与日志视图
...
## Phase 54 — Provider Probe Consistency
...
- [x] **T-58.3** ✅ **验证**：运行 `npm run lint && npm run typecheck && npm run test`，并用真实 `codex exec` 指向本地 gateway 验证请求可成功完成


---

## Phase 59 — Dedicated Codex Endpoint
...
- [x] **T-63.7** 增加 provider 并发上限（`PROVIDER_MAX_INFLIGHT` + 可选覆盖），并补充自动化测试
...
## Phase 64 — Advanced Governance & Usage Controls

- [x] **T-64.1** 增加 token 级限流（`TOKEN_RATE_LIMIT_TPM` / `TOKEN_RATE_LIMIT_BURST` + 覆盖），并补充自动化测试
- [ ] **T-64.2** 增加 API key 日/月配额与预算（`KEY_DAILY_TOKENS` / `KEY_MONTHLY_TOKENS` / `KEY_DAILY_BUDGET_USD` / `KEY_MONTHLY_BUDGET_USD` + 覆盖），并补充自动化测试
- [ ] **T-64.3** 增加内容安全阻断（`CONTENT_BLOCKLIST`），并补充自动化测试
- [ ] **T-64.4** 增加 JWT 用户鉴权与用户/分组配额（`JWT_SECRET` 等），并补充自动化测试
