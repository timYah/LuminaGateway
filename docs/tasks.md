...
- [ ] **T-51.2** 路由与计费更新：模型直通、价格回退、模型不存在切换
- [ ] **T-51.3** Admin API/UI 调整：移除模型映射，补充 provider 定价
- [ ] **T-51.4** 测试更新：移除模型映射测试并覆盖新逻辑
- [ ] **T-51.5** 文档更新：移除模型映射说明，补充定价与 env
- [ ] **T-51.6** ✅ **验证**：运行 `npm run lint && npm run typecheck && npm run test`
