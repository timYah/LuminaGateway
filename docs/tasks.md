...
- [ ] **T-02.1** 安装数据库依赖：`drizzle-orm`、`better-sqlite3`、`drizzle-kit`
- [ ] **T-02.2** 安装类型依赖：`@types/better-sqlite3`
...
- [ ] **T-03.1** 创建 `src/db/index.ts` — 实现 `getDb()` 工厂函数，根据 `DATABASE_TYPE` 环境变量返回 SQLite 或 PostgreSQL 连接
...
- [ ] **T-04.1** 创建 `src/db/seed.ts` — 填充至少 3 个 demo Provider（如 "OpenAI Main"、"Anthropic Backup"、"Third-Party Proxy"）
- [ ] **T-04.2** 在 seed 脚本中为每个 Provider 添加模型映射（至少 5 条，如 gpt-4o、gpt-4o-mini、claude-sonnet-4-20250514、claude-haiku 等）
...
- [ ] **T-05.1** 创建 `src/services/providerService.ts` — 实现 `getAllProviders()` 方法
- [ ] **T-05.2** 实现 `getProviderById(id)` 方法
...
- [ ] **T-06.1** 创建 `src/services/modelService.ts` — 实现 `getModelsBySlug(slug)` 方法，返回匹配 slug 的所有模型记录（含关联 Provider 信息）
...
- [ ] **T-07.1** 创建 `src/middleware/auth.ts` — 实现 Bearer Token 中间件，从 `Authorization` 头提取 token 并与 `GATEWAY_API_KEY` 比对
- [ ] **T-07.2** 鉴权失败返回 `401` 状态码和标准错误体
...
- [ ] **T-08.1** 创建 `src/types/openai.ts` — 定义 OpenAI Chat Completion 请求体类型（model, messages, stream, temperature, max_tokens, tools, tool_choice）
- [ ] **T-08.2** 在 `src/types/openai.ts` 中定义 OpenAI Chat Completion 响应体类型（非流式）
...
- [ ] **T-09.1** 安装依赖：`zod`
- [ ] **T-09.2** 创建 `src/types/validators.ts` — 定义 OpenAI 请求体 Zod schema
...
- [ ] **T-10.1** 创建 `src/routes/openai.ts` — 注册 `POST /v1/chat/completions` 路由，使用 Zod schema 校验请求体，返回 stub 响应
- [ ] **T-10.2** 创建 `src/routes/anthropic.ts` — 注册 `POST /v1/messages` 路由，使用 Zod schema 校验请求体，返回 stub 响应
...
- [ ] **T-11.5** ✅ **验证**：编写 `src/services/__tests__/circuitBreaker.test.ts`，测试开启/过期/重置行为，运行 `npm run test -- circuitBreaker && npm run lint && npm run typecheck`。持续修复直到全部通过。
...
- [ ] **T-13.1** 安装依赖：`ai`、`@ai-sdk/openai`、`@ai-sdk/anthropic`、`@ai-sdk/google`
- [ ] **T-13.2** 创建 `src/services/aiSdkFactory.ts` — 实现 `createAIProvider(provider)` 函数，根据 `provider.protocol` 创建对应的 AI SDK provider 实例（设置 baseURL 和 apiKey）
...
- [ ] **T-14.1** 创建 `src/services/upstreamService.ts` — 实现 `callUpstreamNonStreaming(provider, model, params)` 函数，使用 AI SDK 的 `generateText` 调用上游
...
- [ ] **T-16.1** 创建 `src/services/billingService.ts` — 实现 `calculateCost(inputTokens, outputTokens, inputPrice, outputPrice)` 纯函数
- [ ] **T-16.2** 实现 `billUsage(providerId, modelSlug, usage, model)` 方法 — 计算费用、扣减余额、写入 usageLogs
- [ ] **T-16.3** 处理边界情况：usage 为空时跳过计费、价格为 0 时仅记录日志
...
- [ ] **T-17.5** 所有 Provider 耗尽时返回格式化的错误响应
...
- [ ] **T-18.1** 在 `src/routes/openai.ts` 中替换 stub 响应，接入 `gatewayService.handleRequest()`（非流式路径）
- [ ] **T-18.2** 在 `src/routes/anthropic.ts` 中替换 stub 响应，接入 `gatewayService.handleRequest()`（非流式路径）
...
- [ ] **T-20.1** 在 `src/services/streamRelay.ts` 中实现 `relayAsAnthropicStream(aiSdkStream)` 函数，将 AI SDK 的 stream 转为 `event: content_block_delta\ndata: {...}\n\n` 格式
...
- [ ] **T-22.1** 在 `src/routes/openai.ts` 中添加流式路径：当 `stream: true` 时调用 `handleStreamingRequest()` 并返回 SSE Response
...
- [ ] **T-23.1** 创建 `src/services/protocolConverter.ts` — 实现 `convertOpenAIToUniversal(openaiRequest)` 函数，将 OpenAI 请求体转为 AI SDK 通用参数
...
- [ ] **T-24.1** 在 `src/services/protocolConverter.ts` 中实现 `convertAnthropicToUniversal(anthropicRequest)` 函数 — 处理 `system` 字段的位置映射
...
- [ ] **T-27.1** 创建 `src/middleware/logger.ts` — 实现结构化日志中间件，记录 request ID、method、path、状态码、耗时
- [ ] **T-27.2** 读取 `LOG_LEVEL` 环境变量控制日志级别（debug / info / warn / error）
...
- [ ] **T-29.1** 创建 `src/routes/admin.ts` — 实现 `GET /admin/providers` 路由，返回所有 Provider 列表（含余额、状态）
...
- [ ] **T-32.1** 在 `src/routes/admin.ts` 中实现 `GET /admin/usage` 路由，支持按 providerId、modelSlug、startDate、endDate 过滤
...
- [ ] **T-33.1** 创建 `src/__tests__/integration/` 目录
- [ ] **T-33.2** 编写 `src/__tests__/integration/happyPath.test.ts` — 测试完整非流式请求：auth → 路由 → mock upstream 响应 → 计费 → 正确格式返回
- [ ] **T-33.3** 验证 usageLogs 中有一条记录，Provider 余额已扣减
...
- [ ] **T-35.1** 编写 `src/__tests__/integration/streaming.test.ts` — 测试完整流式请求的 SSE 格式正确性
- [ ] **T-35.2** 测试流式请求完成后计费正确写入
...
- [ ] **T-36.1** 更新 `docs/documentation.md` — 确保所有章节反映实际实现
- [ ] **T-36.2** 更新 `docs/plans.md` — 勾选所有完成的 Milestone，补充决策日志
- [ ] **T-36.3** 创建或更新 `README.md` — 包含项目简介、快速启动、scripts 列表
- [ ] **T-36.4** ✅ **最终验证**：依次运行以下命令，全部通过方可标记项目完成。持续修复直到全部通过。
  ```bash
  npm install
  npm run db:migrate
  npm run db:seed
