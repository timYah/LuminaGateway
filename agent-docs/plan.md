...
- **2026-03-28**: Added Phase 34 to fix zero token displays in `/usage`. The fix unifies HTTP usage persistence by storing both actual `/v1/*` usage and estimated passthrough/convert usage in `usage_logs`, adds explicit `usageSource` metadata for admin visibility, resets existing token statistics during migration, and leaves realtime usage parsing out of scope.
